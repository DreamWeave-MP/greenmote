...
const fn convert_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::DetailedRefDiagnosticsTooltip => {
            "Write full per-reference Unclip diagnostics to greenmote.log. Slower and can create very large log files."
        }
...
const fn dialog_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::SelectMeshGeneratorIni => "Select MeshGenerator INI",
        _ => unreachable!(),
    }
}
