...
const fn convert_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::DetailedRefDiagnostics => "Show detailed ref diagnostics",
        UiText::DetailedRefDiagnosticsTooltip => {
            "Output one diagnostic entry per inspected placed reference instead of only the summary. Includes cell/ref key, terrain contact, static occlusion, and write-status details."
        }
...
const fn dialog_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::ConfirmUnclipWriteMessage => {
            "Unclip will modify the target plugin and create a backup."
        }
        ...
        UiText::DefaultOpenMwUserConfigPath => "Default OpenMW user config path:",
        UiText::UseDefaultPath => "Use default path",
