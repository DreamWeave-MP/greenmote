...
const fn convert_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::DetailedRefDiagnostics => "Detaillierte Ref-Diagnose anzeigen",
        UiText::DetailedRefDiagnosticsTooltip => {
            "Eine Diagnosezeile pro geprüfter platzierter Referenz ausgeben statt nur der Zusammenfassung. Enthält cell/ref-Schlüssel, Terrainkontakt, statische Verdeckung und Schreibstatus."
        }
...
const fn dialog_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::DefaultOpenMwUserConfigPath => "Standardpfad der OpenMW-Benutzerkonfiguration:",
        UiText::UseDefaultPath => "Standardpfad verwenden",
...
const fn settings_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::MeshContactTolerance => "Mesh-Kontakttoleranz",
        UiText::MeshContactToleranceTooltip => {
            "Maximales Meshkontakt/Terrain-Z-Delta, das als bereits auf dem Terrain gilt. Config key: contact_epsilon."
        }
