...
const fn convert_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::DetailedRefDiagnostics => "Mostrar diagnósticos detallados de refs",
        UiText::DetailedRefDiagnosticsTooltip => {
            "Muestra una entrada de diagnóstico por cada referencia colocada inspeccionada en vez de solo el resumen. Incluye clave cell/ref, contacto con terrain, oclusión static y estado de escritura."
        }
...
const fn dialog_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::DefaultOpenMwUserConfigPath => {
            "Ruta predeterminada de configuración de usuario de OpenMW:"
        }
        UiText::UseDefaultPath => "Usar ruta predeterminada",
...
const fn settings_text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::MeshContactTolerance => "Tolerancia de contacto de mesh",
        UiText::MeshContactToleranceTooltip => {
            "Delta máximo de contacto mesh/terrain Z tratado como ya sobre el terrain. Config key: contact_epsilon."
        }
